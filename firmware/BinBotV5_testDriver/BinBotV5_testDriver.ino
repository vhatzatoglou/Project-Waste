...
if (digitalRead(STATUSSCALE_PIN) == Locker_Closed)
{
  ...
  readscale1();
    {
      ...
      }
      {
        ...
        WeightList.clear();
        Serial.println("weightAt -->" + String(weightA));
      } 
    }
}
 ...
 // Serial.println(("SENSOR2 " + String(digitalRead(SENSOR2))));
  ...
  Serial.println("****************     STARTING   BINBOT SYSTEM  ver 3.0     ***************");
...
void loop()
{
  ...
  if (LockStatus == 0)
  {
    ...
    if (digitalRead(STATUSSCALE_PIN) == Locker_Closed && lc == 0 )
    {
      Working_time=millis()- Working_time;
    ...
    //δεν ανοιξε ο κάδος autostart send stored data 
    // if (millis() - start_time > 35000 && System_Tag ==0 && mustUnlock==0)
    // if (digitalRead(STATUSSCALE_PIN) == Locker_Closed && lc == 0 && gr >-1)
    // {
    //   Serial.println("TRANSMIT STORED DATA");
    //   beep(1);
    //   //transmit_data();
    //   working_period = 0;
    //   done();
  ...
  if (Mainntanace == 1)
  {
   ...
   //δεν έκλεισε ο δημότης τον κάδο πάνω απο 50 δευτερόλεπτα
  if (millis() - start_time > 50000)
    if (digitalRead(STATUSSCALE_PIN) == Locker_Opened  && Mainntanace == 0 && Garbagecollection == 0)//&& lc == 1
    {
      ...
      if (millis() - start_time > 40000 && millis() - start_time < 60000)
        beepDelay(1500);
