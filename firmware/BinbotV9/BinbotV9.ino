...
int chgTimes = 0;
int chgTimesNew = 2;
int p, DAY1, MONTH_, YEAR_, YEAR1, Seconds_, Hours_, Minutes_, hourOfDay=0, DAY_OF_WEEK, hasModemSIM, contentLength;
boolean GPRS_on;
int sp=0;
bool reply = false;
...
#define CHARGING_POWER_OFF 19     //OUTPUT CHARGING ON/OFF 
#define CHARGING_POWER_ON 33     //OUTPUT CHARGING ON/OFF 
  ...
  if (hasModemSIM > 0) //&& WiFi.status()!= WL_CONNECTED
  {
    ...
    //  delay(200);
      ...
      if (datetimeSerialAT.substring(0, 2).toInt() >= 19)
      {
        ...
        Serial.println("hourOfDay: " + String(hourOfDay));
...
void checkBootReason()
{
  ...
  Serial.println( "resetReason " + String(resetReason));
  ...
  Serial.println( "Reset Reason Softaware");
...
void blinkGreen(void *pvParameters)
{
  ...
  for (;;)
  {
   ...
   if ( StartWeighting()  && wa ==20 && tagId!="" &&  (millis() - start_time)>15000)
   {
    ...
    check_battery();
    if (( (tagId!="") || Garbagecollection==1) &&  millis()- delayTimer>2000 )
    {
       delayTimer=millis();
       CalculateCoverRange();
     }

    if (tagId=="" && transmitData==0 )
    {
  ...
  else
  {
   ...
   // if  (Garbagecollection == 1) modem_on();
  }
...
void getSettings()
{
  ...
  if (f.available())
  {
    ...
    // Serial.println("Servo_i : " + String(Servo_i));
      ...
      // if(CHARGING_ON>=4)
      // {
      //   pinMode(CHARGING_POWER_OFF, OUTPUT);
      //   digitalWrite(CHARGING_POWER_OFF, HIGH); 
      //   delay(100);
      //   pinMode(CHARGING_POWER_OFF, INPUT);
      //   CHARGING_ON=0;
      // }
      // if(CHARGING_ON==2)
      // {
      //   pinMode(CHARGING_POWER_ON, OUTPUT);
      //   digitalWrite(CHARGING_POWER_ON, HIGH); 
      //   delay(100);
      //   pinMode(CHARGING_POWER_ON, INPUT);
      // }
      Serial.println("==========================================================");
    }
...
{ 
  ...
  if (   I2cbusy==1  ||  (Sen1==0)  ) return;
        ...
        dist1 = Cover_sensor.rangePollMeasurement();
        // dist1 =dist1 + Cover_sensor.rangePollMeasurement();
        // dist1=dist1/2;
        CheckCoverSTatus();
...
int CheckCoverSTatus()
{
  ...
  if (dist1<17 &&  CoverStatus!=Locker_Closed)//17-80
    { 
       ...
       if (checkClosed==2)
       {
         CoverStatus=Locker_Closed;
          checkClosed=0;
          checkOpened=0;
          if ( CoverStatus==Locker_Closed  &&  ( Garbagecollection == 0) && Lockstatus == 1)
          lockByServo();
       }

       return CoverStatus;
    }
   if (dist1>25 && dist1<=255 &&  CoverStatus==Locker_Closed )//25-100
   {
...
boolean checkFullBin()
{
  ...
  if (dist2>160)
    ...
    I2cbusy=0;
  if ( dist1<150 &&  dist2<150 &&  dist1>50 &&  dist2>50)
    return true;
  else
    return false;
}
...
void done()
{
  ...
  //if (Charging_time>0) ESP.restart();
...
void ReadAccelerometer()
{
  ...
  if (NewVersion==0 ||    I2cbusy==1 || Sen5==0 ||  transmitData==1 )
   return;
   
   // read raw accel/gyro measurements from device
  
 if ((millis() - acc_time) >3000 )
  {
    I2cbusy=1;
...
void AirPollution() {
  if (   I2cbusy==1 ||   (Sen7==0) ||  transmitData==1  ) return;
  I2cbusy=1;
...
float getDistanceFromRSSI(int rssi, int txPower = -59) {
  return pow(10.0, ((float)(txPower - rssi)) / (10.0 * 2.0));
}
void bleScanning()
{
...
if (device.haveServiceData()) {
  ...
  float distance = getDistanceFromRSSI(rssi);
   ...
   const uint8_t expected[] = {0x12, 0x34, 0x49, 0x44, 0x30, 0x30, 0x31}; // ID001
      if (data.length() >= 5 &&
          memcmp(data.data(), expected, 7) == 0 && rssi>-67)
      {
        Serial.println("✅ Found BinBot Service");
        if (device.haveServiceUUID()) {
        BLEUUID uuid = device.getServiceUUID();
        if (uuid.toString()=="00000df1-0000-1000-9000-00805f9b34fb"){
              tagId=uuid.toString().c_str();
              tagId.replace("-","");
...
Serial.println("🔍Ble Scanning...");
 //if (Sen6==1) 
 while ((millis() - start_time )< 15000 && tagId == "" && Garbagecollection==0)
 {
  readNFC();
  ReadAccelerometer();
  bleScanning();
